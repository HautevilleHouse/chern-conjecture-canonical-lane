...
In-paper anchor: `paper/CHERN_CONJECTURE_PREPRINT.md` (`CHN_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving affine-completeness and Euler-characteristic persistence in the declared admissible class through an admissible affine-connection closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`CHN_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_CHN`.
...
If near-failure extraction and no-Zeno spacing hold, then `CHN_G3` prevents hidden drift before rigidity is applied.
