...
In-paper anchor: `paper/CHERN_CONJECTURE_PREPRINT.md` (`CHN_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving affine-completeness and Euler-characteristic persistence in the declared admissible class through an admissible affine-connection closure architecture`.

## Objects

- admissible class: the declared class `A` or routed admissible lattice in the main preprint.
- canonical/base object package: let `u_tau = (K_tau, M_tau, D_tau, N_tau, L_tau)` denote the admissible state of affine-connection packets, affine-connection data, defect ledgers, normalization parameters, and endpoint locks.
...
`CHN_G1` closes when `kappa_canonical` satisfies the response-floor requirement: projected affine-connection response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_CHN`.
...
Payload: check that all quantities used by `kappa_canonical` are defined on the projected sector named in the main preprint.
...
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_canonical`, then `CHN_G1` supplies the projected/protected-core input to `M_CHN`.
