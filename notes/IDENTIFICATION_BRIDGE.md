...
In-paper anchor: `paper/CHERN_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving affine-completeness and Euler-characteristic persistence in the declared admissible class through an admissible affine-connection closure architecture`.
...
2. the endpoint-transfer lock `chern_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_CHN` remains positive.
...
Payload: verify `chern_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_CHN` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `chern_transfer`
