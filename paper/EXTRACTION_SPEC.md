...
- `sigma_deformation` from `deformation_floor_raw - connection_loss_raw - restart_loss_raw`
- `kappa_compact` from `1.0 / (1.0 + delta_comp_sup_raw)`
- `rho_rigidity` from `rho_rigidity_raw`
- `chern_transfer` from `c_transfer_raw * transfer_gain_raw - e_transfer_raw`
...
3. `scripts/chn_closure_guard.py`
